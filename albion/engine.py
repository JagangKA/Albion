...
def _trusted(item_id: str, rec: dict) -> bool:
    ...
    В дампе артефакты выглядят как «50 рун → артефакт», что даёт маржу в тысячи
    процентов. В игре так не работает — иначе руны давно стоили бы дороже.
    Модель неполная, поэтому такие позиции из расчёта убираем, чтобы они
    не выдавались за заработок.
    """
